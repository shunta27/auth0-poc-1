...
sequenceDiagram
    ...
    U->>B: https://localhost:3000 にアクセス
    B->>A: GET /
...
1. **HTTPS 必須**: 開発環境でも `https://localhost:3000` を使用
2. **メール検証必須**: ユーザーは email_verified=true になるまでログインできない
3. **JWT 有効期限**: 検証トークンは 24 時間で無効化
4. **ロールバック機能**: メール送信失敗時は Auth0 からユーザーを削除
