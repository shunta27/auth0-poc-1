...
Next.js 15.3.4 + Auth0 を使用した認証機能付きPoCアプリケーション。カスタムユーザー登録とメール検証機能を含む。

## システム構成

### 開発環境
- **URL**: `https://127.0.0.1:3000` (localhost ではなく 127.0.0.1 を使用)
- **プロトコル**: HTTPS必須 (Auth0認証に必要)
...
| ファイル | 役割 | 説明 |
|---------|------|------|
| `lib/auth0.ts` | Auth0クライアント設定 | メインのAuth0 SDK設定 |
...
const verificationToken = jwt.sign(
  { email },
  JWT_SECRET,
  { expiresIn: '24h', issuer: 'auth-poc', subject: email }
);
...
return NextResponse.redirect(
  `${APP_BASE_URL}/auth/login?login_hint=${email}`
);
```
...
1. **ログインループ**
   - `127.0.0.1` ではなく `localhost` を使用している
